...
export default class PrivacyPolicy extends Component {
  ...
  getPrivacyPolicy = async () => {
    ...
    let response = await ApiCall.get(Url.GET_PRIVACY_POLICY, await config());
    if (response.status === 200) {
      this.setState({
        content: response.data.privacyPolicy.policy,
        entityId: response.data.privacyPolicy.id,
        languages: response.data.languages,
        translations: response.data.privacyPolicy.translations,
        spinning: false,
      });
    }
  };
  ...
  toggle = (item) => {
    ...
    if (item.id) {
      this.setState({
        translationContent: item.body[0].policy,
  ...
  storeTranslation = async () => {
    ...
    if (validation.status) {
      ...
      let response = await ApiCall.post(
        Url.STORE_PRIVACY_POLICY_TRANSLATIONS,
        {
  ...
  updateTranslation = async () => {
    const { selectedLanguage, translationContent, entityId, translationId } =
      this.state;
    let validation = this.handleValidation();
    if (validation.status) {
      this.setState({ loadingApi: true });
      let response = await ApiCall.post(
        Url.UPDATE_PRIVACY_POLICY_TRANSLATIONS,
        {
          language_id: selectedLanguage,
          entity_id: entityId,
          policy: translationContent,
              ...
              <Card>
                ...
                <CardBody>
                  ...
                  <Form
                    ...
                    onSubmit={this.updatePrivacyPolicy}
                  >
                    <FormGroup row>
                      {/*<Label sm="3">*/}
                  ...
                  <div style={{ marginTop: "100px" }}>
                    <CardTitle>
                      {/*<IntlMessages id="good-student.create-good" />*/}
                      Translations |{" "}
                      <Button size="xs" color="primary" onClick={this.toggle}>
                        Add
                      </Button>
                    </CardTitle>
                    <DataTradesTable
                            ...
                            return (
                              <div>
                                ...
                                <Button
                                  ...
                                  className="mb-2"
                                  onClick={() =>
                                    this.deleteTranslation(rowData.id)
                                  }
                                >
                    ...
                    {this.state.showModal && (
                      <TranslationsModal
                        ...
                        handleChangeTranslationContent={
                          this.handleChangeTranslationContent
                        }
