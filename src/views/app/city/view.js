...
export default class RolesView extends Component {
    ...
    getAllCities = async ()=> {
        ...
        let response = await ApiCall.get(Url.ALL_CITIES, await config())
        // console.log(response)
        if(response.status=== 200){
            this.setState({spinning: false});
            this.setState({
                cities: response.data.cities,
                allCities:  JSON.parse(JSON.stringify(response.data.cities))
            });
        }
    };
    toggle = (permissions) => {
    ...
    getData =() => {
        const { displayLength, page, allCities } = this.state;
        return allCities.filter((v, i) => {
    ...
    handleFilterChange = (e) => {
        ...
        const data = this.state.cities.filter((v, i) => {
        ...
        const filteredData = data.filter(item => {
            ...
            return (
                ...
                // ||
                // item.province.name.toLowerCase().indexOf(query) >= 0
            )
        });
        this.setState({
            allCities: filteredData
        })
        if(e.target.value=== ''){
            this.setState({
                allCities: this.state.cities
            })
        }
    };
    ...
    confirmChangeStatus = async (item) => {
        this.setState({spinning: true});
        let response = await ApiCall.get(`${Url.DELETE_CITY}/${item}`, await config());
        if(response.status === 200){
            this.setState({spinning: false});
            this.getAllCities();
                ...
                <Row>
                    <Colxx xxs="12">
                        <div className="text-zero top-right-button-container">
                            <Link to='/app/city/create'><Button size='lg' color={'primary'}> <IntlMessages id="create" /></Button></Link>
                        </div>
                        <Breadcrumb heading="City" match={this.props.match} />
                            ...
                            <Card className="h-100">
                                <CardBody>
                                                ...
                                                {(rowData, rowIndex) => {
                                                    return <div>
                                                        ...
                                                        <Button color="secondary" size="xs" className="mb-2">
                                                            <Link to={`/app/city/edit/${rowData._id}`} style={{color: 'white'}}><IntlMessages id="client.edit" /></Link>
                                                        </Button>}

                                                        {" "}{" "}

                                                        {this.state.userPermissions.find(item => item.name === "city.delete") &&
                                    ...
                                    <Pagination
                                        ...
                                        total={this.state.allCities.length}
