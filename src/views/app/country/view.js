...
export default class PermissionsView extends Component {
    ...
    getAllProvinces = async (item)=> {
        ...
        if(response.status=== 200){
            ...
            this.setState({
                permissions: response.data.countries.reverse(),
                allPermissions: JSON.parse(JSON.stringify(response.data.countries))
            });
        }
    };
    ...
    confirmChangeStatus = async (item) => {
        ...
        let response = await ApiCall.get(`${Url.DELETE_COUNTRY}/${item}`, await config());
        if(response.status === 200){
            this.setState({spinning: false});
            this.getAllProvinces();
                ...
                <Row>
                    <Colxx xxs="12">
                        <div className="text-zero top-right-button-container">
                            <Link to='/app/country/create'><Button size='lg' color={'primary'}> <IntlMessages id="create" /></Button></Link>
                        </div>
                                                ...
                                                {(rowData, rowIndex) => {
                                                    return <div>
                                                        ...
                                                        <Button color="secondary" size="xs" className="mb-2">
                                                            <Link to={`/app/country/edit/${rowData._id}`} style={{color: 'white'}}><IntlMessages id="permission.edit" /></Link>
                                                        </Button>}
