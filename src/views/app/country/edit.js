...
export default class UpdatePermission extends Component {
    ...
    getSinglePermissionData = async ()=> {
        ...
        let response = await ApiCall.get(`${Url.EDIT_COUNTRY}/${this.props.match.params.id}`, await config());
        if(response.status === 200){
            this.setState({
                name: response.data.country.name,
                id: response.data.country._id,
                spinning: false
            });
        }
    }
    ...
    updatePermission = async (e)=> {
        ...
        let response = await ApiCall.post(`${Url.UPDATE_COUNTRY}/${this.props.match.params.id}`, {
            name: name,
            id: this.props.match.params.id
        }, await config());
        if(response.status === 200){
            this.setState(initialState);
            this.props.history.push('/app/country/view')
                ...
                <Row>
                    <Colxx xxs="12">
                        <div className="text-zero top-right-button-container">
                            <Link to='/app/country/view'><Button size='lg' color={'secondary'}>Cancel</Button></Link>
                        </div>
